...
- [ ] **Step 1: Record the current status and diff**
...
- [ ] **Step 2: Preserve the approved HUD direction**
...
- [ ] **Step 3: Create the remediation worktree**
...
- [ ] Execute every checkbox in Package A.
- [ ] Run the package gate.
- [ ] Review all SQL for nonnegative constraints, lock ordering, and rollback behavior.
- [ ] Record Package A's final migration number and compatibility versions in this plan's progress notes.
...
- [ ] Rebase Package B on accepted Package A.
- [ ] Assign the next migration number after A.
- [ ] Execute every checkbox in Package B.
- [ ] Confirm no game route calls `syncOpenTasks` or `syncRumors`.
- [ ] Confirm AI calls occur outside database transactions.
