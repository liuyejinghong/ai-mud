...
## 6. 2026-09-26 MVP 试玩前打磨补充

本轮沿用同一教程版本授权与草稿分支。隔离 PostgreSQL 原档在第二阵列运输自然完成后，安装从基地 11:46 到 12:06 停在 22/80：施工机器人均低于 500Wh，已结束运输的闲置驮运按数据库返回顺序先吃尽充电预算。修复目标是恢复真实施工和可理解的玩家反馈，不增加玩法、第二套教程状态、公开 API、迁移或上线范围。

| 归属 | 冻结行为与可复验样例 |
|---|---|
| 行业规则 | 仅在充电分配阶段，优先给当前可推进项目步骤所需且低于出工门槛的闲置机组分配有限电力，再给其它可充电设备。无关闲置设备的输入顺序不得让当前工序机组长期得不到电。四台闲置驮运排在一台 480Wh 的当前安装机组之前、夜间有限预算时，首分钟后该机组应获得电量、随后可按原规则出工；交换设备输入顺序不改变此结果。仍保留基础/施工/制造/充电用电次序、总预算、电池容量、工作耗电、原子结算和低电本分钟不出工。 |
| 玩家界面 | 用现有快照显示购前的缺口、小计/总价和已知到货条件；失败原因出现在操作点附近；协作选择只比较可证的电量与等待条件，历史保留已有接手者；零出工按实际电量/暂停/工序状态解释，不编造工期；首工程完成、第二阵列真实开工后明确收束六段引导，并如实说明第二工程仍在施工。两项都完工时另显示完工事实。充电位数字若未参与结算，不向玩家宣称为已生效限制。 |
| 所有权 | 行业实现与同名纯函数测试只由充电线写；`apps/web/src/features/base/` 及同名测试只由界面线写；本合同、`packages/shared/src/version.ts` 与版本测试只由集成者写。无需修改 shared DTO、迁移、架构 allowlist 或锁文件。独立验收只读集成代码，在隔离数据库运行。 |

本轮规则结算语义变化递增 `rulesetVersion`，`PRODUCT_VERSION` 留待正式发布决定。集成后须在同一 HEAD 通过定向测试、全量 typecheck/test/build/arch 和真实 PostgreSQL 回归；再用新废档走到第二阵列运输、安装、验收，并记录真人盲玩的独立缺口。通过测试、文档或 AI 浏览器试玩均不自动等于 `PLAYTEST_PASS`，也不授权合并或部署。
