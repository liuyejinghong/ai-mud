...
const evidenceDir = "/private/tmp/yudian-tutorial-browser";

if (!email || !password) {
  throw new Error("The real PostgreSQL E2E runner must provide player credentials.");
}

test("新玩家能看见下一步、实际开工并在同档刷新后保留回执", async ({ page }) => {
  ...
  await page.screenshot({ path: `${evidenceDir}/01-intro-1200.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${evidenceDir}/02-goal-1200x701.png`, fullPage: true });

  await page.setViewportSize({ width: 720, height: 450 });
  await expect(goal).toBeInViewport();
  await expect(action).toBeInViewport();
  await page.screenshot({ path: `${evidenceDir}/03-goal-720x450.png`, fullPage: true });

  await page.setViewportSize({ width: 1200, height: 701 });
  await page.evaluate(() => { document.documentElement.style.zoom = "200%"; });
  await expect(goal).toBeInViewport();
  await expect(action).toBeInViewport();
  await page.screenshot({ path: `${evidenceDir}/04-goal-zoom200.png`, fullPage: true });
  ...
  await expect(goal).toContainText("安装运抵的太阳能设施");
  await page.screenshot({ path: `${evidenceDir}/05-project-receipt.png`, fullPage: true });
  ...
  await page.screenshot({ path: `${evidenceDir}/06-same-save-refresh.png`, fullPage: true });
